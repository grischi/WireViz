...
class Options:
    ...
    bgcolor_conduit: Optional[SingleColor] = None
...
class Cable(TopLevelGraphicalComponent):
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self, Conduit):
            ...
            # Allow Conduit instances to be created without colors here.
            # Colors will be populated later when cables are assigned to the conduit.
            if not self.colors:
                self.colors = []
                self.wirecount = 0
            else:
...
class Conduit(Cable):
    ...
    def get_port(self, cable: Cable, port: int) -> int:
        # ensure an entry exists for this cable designator
        if cable.designator not in self.cableports:
            self.cableports[cable.designator] = []

        # if port already assigned and non-zero, return it
        if len(self.cableports[cable.designator]) >= port and self.cableports[cable.designator][port - 1]:
            return self.cableports[cable.designator][port - 1]

        # extend list to required port length with zeros
        if len(self.cableports[cable.designator]) < port:
            self.cableports[cable.designator].extend([0] * (port - len(self.cableports[cable.designator])))

        # allocate new conduit port
        self.ports += 1
        self.cableports[cable.designator][port - 1] = self.ports

        # copy color from cable if available, else use empty placeholder
        if cable.colors and len(cable.colors) >= port:
